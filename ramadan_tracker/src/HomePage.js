...
export default function HomePage() {
  ...
  return (
    <div className={`full_page img_bg shade1 image-${imageIndex}`}>
      <div>
        <div className="row">
          <button className="shade4" onClick={handleImageChange}>
            Change Image
          </button>
          <br />
        </div>
        <div className="row display-4">
          Now: {time.getHours() > 12 ? time.getHours() - 12 : time.getHours()}:
          {time.getMinutes()}:{time.getSeconds()} {AMPM}
        </div>
        <Alarm2/>
      </div>
    </div>
